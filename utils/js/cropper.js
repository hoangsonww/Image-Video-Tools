  ...
  reader.onload = (event) => {
    img.onload = () => {
      if (img.width < MIN_IMAGE_DIMENSION || img.height < MIN_IMAGE_DIMENSION) {
        alert(
          "Image is too small to be cropped. Please upload an image with a minimum dimension of 700x700."
        );
        ...
        canvas.width = Math.max(
          Math.min(window.innerWidth * 0.9, img.width),
...
downloadBtn.addEventListener("click", () => {
  if (
    ...
    tempCtx.drawImage(
      img,
      rectStartX * (img.width / canvas.width),
      rectStartY * (img.height / canvas.height),
      croppedWidth * (img.width / canvas.width),
      croppedHeight * (img.height / canvas.height),
      0,
      0,
      croppedWidth,
      croppedHeight
