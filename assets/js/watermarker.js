  ...
  reader.onload = (event) => {
    img.onload = () => {
      if (img.width < MIN_IMAGE_DIMENSION || img.height < MIN_IMAGE_DIMENSION) {
        alert(
          "Image is too small. Please upload an image with dimensions at least 100x100 pixels."
        );
...
watermarkBtn.addEventListener("click", () => {
  ...
  if (xPos < 0 || xPos > canvas.width || yPos < 0 || yPos > canvas.height) {
    alert(
      `Please enter valid positions. Maximum X: ${canvas.width}, Maximum Y: ${canvas.height}`
    );
    return;
  }
